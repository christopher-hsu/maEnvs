...
setTrackingEnv4 : Double Integrator Target model with KF belief tracker
    obs state: ([d, alpha, ddot, alphadot, logdet(Sigma), o_d, o_alpha],([d, alpha] * nb_agents-1)) *nb_targets
            where nb_targets vary between a range and makes the obs state a 2d matrix
            num_targets describes the upperbound on possible number of targets in env
            num_agents and nb_agents is fixed
    Target : Double Integrator model, [x,y,xdot,ydot]
    Belief Target : KF, Double Integrator model

"""

class setTrackingEnv4(maTrackingBase):
    ...
    def __init__(self, num_agents=1, num_targets=2, map_name='empty', 
        ...
        self.limit['state'] = [np.concatenate(([0.0, -np.pi, -rel_vel_limit, -10*np.pi, -50.0, 0.0], 
                                np.concatenate(([0.0, -np.pi ],[0.0, -np.pi]*(self.num_agents-1))))),
                                np.concatenate(([600.0, np.pi, rel_vel_limit, 10*np.pi,  50.0, 2.0], 
                                np.concatenate(([self.sensor_r, np.pi],[600.0, np.pi]*(self.num_agents-1)))))]
            ...
            for p, ids in enumerate(obs_dict):
                if agent_id != ids:
                    ...
                    for n in range(self.nb_targets):
                        obs_dict[agent_id][n].extend([r,alpha])
