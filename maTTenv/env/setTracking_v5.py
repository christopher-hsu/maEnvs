...
setTrackingEnv3 : Double Integrator Target model with KF belief tracker
    obs state: [d, alpha, ddot, alphadot, logdet(Sigma), o_d, o_alpha, x, y, theta] *nb_targets
            x, y, theta describe the learner's current frame
...
class setTrackingEnv5(maTrackingBase):
    ...
    def __init__(self, num_agents=1, num_targets=2, map_name='empty', 
        ...
        self.limit['state'] = [np.concatenate(([0.0, -np.pi, -rel_vel_limit, -10*np.pi, -50.0, 0.0], [0.0, -np.pi, 0.0, 0.0, -np.pi])),
                               np.concatenate(([600.0, np.pi, rel_vel_limit, 10*np.pi,  50.0, 2.0], [self.sensor_r, np.pi, 600, 600, np.pi]))]
        ...
        for jj in range(self.nb_targets):
            for kk in range(self.nb_agents):
                ...
                obs_dict[self.agents[kk].agent_id].append([r, alpha, 0.0, 0.0, logdetcov, 
                                                           ...
                                                           self.agents[kk].state[0],
                                                           self.agents[kk].state[1],
                                                           self.agents[kk].state[2]])
        ...
        for ii, agent_id in enumerate(action_dict):
            ...
            for kk in range(self.nb_targets):
                ...
                obs_dict[agent_id].append([r_b, alpha_b, r_dot_b, alpha_dot_b,
                                        ...
                                        self.agents[ii].state[0],
                                        self.agents[ii].state[1],
                                        self.agents[ii].state[2]])
            obs_dict[agent_id] = np.asarray(obs_dict[agent_id])
